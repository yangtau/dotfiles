...
# The original version is written by Shrey Choudhary(@SxC97), https://github.com/SxC97/Yabai-Spaces
...
FONT=' | font=JetBrains Mono Regular | trim=true | size=14'
...
if [ "$STYLE" = "NUMBER" ]; then  
  ...
  while [ $i -le $TOTAL ] || [ $i -eq $TOTAL ]; do
    if [ $i -eq $CURRENT ]; then
      STRING+="$COLOR$SELECTLEFT${SELECTED}$SELECTRIGHT$DEFAULT"
      i=$(( $i + 1 ))
    else
